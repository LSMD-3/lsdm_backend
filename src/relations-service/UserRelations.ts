...
// const RelationService = new Neo4jClient({ path: 'bolt://3.83.230.93:7687', username: 'neo4j', password: 'events-language-components' })
...
class Neo4jService {
  ...
  public async addRestaurantIfNotExist(restaurant: string) {
    const session = Neo4jClient.session()
    const exist = (await session.run(`MATCH  (r:Restaurant {id:"${restaurant}"}) RETURN r`)).records.length
    if (!exist) await session.run(`CREATE  (r:Restaurant {id:"${restaurant}"}) RETURN r`)
    session.close()
  }
